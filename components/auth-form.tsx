...
export function AuthForm({ mode, initialReferralCode = "" }: { mode: Mode; initialReferralCode?: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === "register") {
        ...
        window.location.assign(profile.permissions.includes("users.read") ? "/admin" : "/dashboard");
  ...
  return <ThemeSurface className="rw-auth-surface"><div className="auth-layout"><aside className="auth-story"><Brand /><div className="auth-story-content"><span className="eyebrow">SMALL STEPS. REAL POSSIBILITY.</span><h1>A little time.<br />A whole lot<br />of potential<span>.</span></h1><p>Discover approved tasks, share your perspective,<br />and build towards your next reward.</p><div className="auth-coin"><Layers2 size={80} /></div><span className="auth-story-check"><ShieldCheck size={19} />Verified tasks. Transparent rewards.</span></div><span className="auth-copyright">Make your time count.</span></aside><main className="auth-main"><div className="rw-auth-top"><Link className="back-link" href="/"><ArrowLeft size={16} />Back to home</Link><ThemeToggle /></div><div className="rw-auth-mobile-brand"><Brand /></div><div className="auth-form-wrap"><span className="auth-step">YOUR REWARDLY ACCOUNT</span><h2>{copy[mode][0]}</h2><p>{copy[mode][1]}</p><form onSubmit={submit}>
    ...
    <button className="button dark full-width" disabled={busy}>{busy ? <><span className="rw-spinner" />Please wait…</> : copy[mode][2]}<ArrowRight size={17} /></button>
  </form><div className="auth-switch">{mode === "login" ? <>New to Rewardly? <Link href="/register">Create an account</Link><p><Link href="/verify-email">Need to verify your email?</Link></p></> : <>Already have an account? <Link href="/login">Sign in</Link></>}</div><div className="auth-secure"><ShieldCheck size={15} />Your account, protected.</div></div><span className="auth-bottom">Rewardly · Foundation release</span></main></div></ThemeSurface>;
}
