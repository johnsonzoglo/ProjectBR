...
import { ArrowRight, ArrowUpRight, Check, ChevronRight, Clock3, Coins, Crown, Gift, ShieldCheck, Sparkles, Zap } from "lucide-react";
...
export function MembershipCard({ preview }: { preview: boolean }) {
  return <RewardCard className="rw-membership-card"><span className="rw-premium-symbol"><Crown size={24} /></span><div><span className="rw-overline">A LITTLE EXTRA</span><h2>More room to grow.</h2><p>Explore what’s next with membership.</p></div><Link href="/membership" className="rw-premium-link">{preview ? "Preview benefits" : "Explore membership"}<ArrowUpRight size={16} /></Link></RewardCard>;
}
