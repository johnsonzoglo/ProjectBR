...
function ProfileContent({ profile }: { profile: Profile }) {
  ...
  return <Shell name={savedName} admin={profile.permissions.includes("users.read")}>
    <div className="page-heading"><div><div className="eyebrow">MAKE IT YOURS</div><h1>Your account.</h1><p>Your details, security, and sessions. All in your hands.</p></div><span className="status-badge">{profile.user.emailVerified ? "Email verified" : "Email not verified"}</span></div>
    <section className="rw-profile-hero"><div className="rw-profile-avatar">{avatar ? <img src={avatar} alt="Your profile" /> : savedName.slice(0, 1).toUpperCase()}</div><div className="rw-profile-identity"><h2>{savedName}</h2><span className="status-badge"><ShieldCheck size={15} />{profile.user.emailVerified ? "Verified email" : "Active member"}</span><p>{profile.user.email}</p></div><div className="rw-profile-status"><small>MEMBER SINCE</small><strong>{new Date(profile.user.createdAt).toLocaleDateString(undefined, { month: "short", year: "numeric" })}</strong></div><div className="rw-profile-progress"><span>Your task progress</span><strong>{tasks.data ? `${completed.length} / ${tasks.data.length}` : "—"} tasks</strong><progress value={completed.length} max={tasks.data?.length || 1} aria-label="Completed tasks out of available task history" /></div></section>
    ...
    <div className="profile-grid">
      <section className="panel"><div className="rw-panel-kicker">PERSONALIZE</div><h2>Personal details</h2><p>A name that feels like you.</p><form className="profile-form" onSubmit={save}><label>Display name<input value={name} onChange={e => setName(e.target.value)} required minLength={2} maxLength={80} autoComplete="name" /></label><label>Email address<input value={profile.user.email} readOnly /></label><label>Mobile number (optional)<input type="tel" value={phone} onChange={e => setPhone(e.target.value)} maxLength={32} autoComplete="tel" /></label><label>Profile photo<input type="file" accept="image/jpeg,image/png,image/webp" onChange={async e => { const file=e.target.files?.[0]; if(file) try { setAvatar(await designImageData(file,320)); setError(""); } catch(f) { setError((f as Error).message); } }} /><small>Compressed on your device before upload.</small></label><button className="button dark" disabled={busy}>{busy ? "Saving…" : "Save changes"}</button></form><div className="account-detail"><span>Member since</span><strong>{new Date(profile.user.createdAt).toLocaleDateString()}</strong></div></section>
      ...
      <section className="panel"><div className="rw-panel-kicker">YOUR DEVICES</div><h2>Active sessions</h2><p>Sign out any session you no longer recognize.</p>{sessions.map(s => <div className="session-row" key={s.id}><span className={`rw-session-dot ${s.current ? "current" : ""}`} /><div><strong>{s.current ? "This session" : "Another session"}</strong><p>{s.userAgent?.slice(0, 65) || "Unknown browser"}</p><p>Started {new Date(s.createdAt).toLocaleDateString()} � {s.ipAddress || "IP unavailable"}</p></div><button onClick={() => revoke(s.id, s.current)}>Sign out</button></div>)}</section>
    ...
    </div><AccountTools profile={profile}/>
  </Shell>;
}
