...
export async function notificationFeed(userId: string, permissions: string[]) {
 ...
 if(staff){
  ...
  if(permissions.includes("audit.read")) {const failures=await db.auditLog.findMany({where:{action:"email.delivery_failed",createdAt:{gte:since}},select:{id:true,reason:true,createdAt:true},orderBy:{createdAt:"desc"},take:100});const seen=new Set<string>();failures.forEach(r=>{const message=r.reason||"Check SMTP configuration and the account email address.";if(seen.has(message))return;seen.add(message);add("email:"+r.id,"Account email failed",message,"/admin/audit","email",r.createdAt);});}
 }else{
  await Promise.all([
